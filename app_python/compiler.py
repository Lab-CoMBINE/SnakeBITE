...
def nangenline_compiler(ops, ann_vec, work_dir, cst_name, cores):
    ...
    snake_cmd = f"nohup snakemake --use-conda --cores {cores} --printshellcmds --keep-going --configfile={work_dir}/{cst_name}/config.yaml {outputs} >> {work_dir}/{cst_name}/log.out"
    return snake_cmd