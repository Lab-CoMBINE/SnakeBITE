...
def explore_ssh_server(host, port, username, password):
    process = subprocess.Popen(["python", "filexplorer_ssh.py", host, port, username, password], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def ref_dtpt(ssh, usr, psswrd, save_ip, save_usr, ref, ref_txt, data, config):
    ui.modal_remove()
