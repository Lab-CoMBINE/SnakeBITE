...
#Definition of app.py and config.yaml absolute path
actual_dir = os.path.dirname(os.path.abspath(__file__))
config_path = os.path.join(actual_dir, '..', 'config.yaml')
workflow_dir = os.path.join(actual_dir, '..', 'workflow')
...
app_ui = ui.page_fluid(
    ...
    ui.navset_pill_list(
        #INSTALLATION SETUP UI PAGE
        ui.nav("Setup", ui.h1("Tool Usage Setup"), ui.hr(),
               ui.row(ui.column(4, "Select the tool for Dorado usage: "), ui.column(2, ui.input_action_button("dorado_usage", "Dorado Datapath")), ui.column(2,"Launch tool from: "), ui.column(4, ui.output_text_verbatim("text_doradousage_dtpt"))),
               ui.row(ui.column(4, "Select the folder with Dorado models: "), ui.column(2, ui.input_action_button("dorado_mdl_folder", "Dorado Model Folder")), ui.column(2,"Dorado models in: "), ui.column(4, ui.output_text_verbatim("text_doradomdlfold_dtpt"))),
               ui.hr(),
               ui.row(ui.column(4, "Select the tool for Clair3 usage: "), ui.column(2, ui.input_action_button("clair3_usage", "Clair3 Datapath")), ui.column(2,"Launch tool from: "), ui.column(4, ui.output_text_verbatim("text_clair3usage_dtpt"))),
               ui.hr(),
               ui.row(ui.column(4, "Select the folder in which Gasoline is installed: "), ui.column(2, ui.input_action_button("gasoline_usage", "Gasoline Folder Datapath")), ui.column(2,"Launch tool from: "), ui.column(4, ui.output_text_verbatim("text_gasolineusage_dtpt"))),
               ui.hr(),
               ui.row(ui.column(4, "Select the tool for annotsv usage: "), ui.column(2, ui.input_action_button("annotsv_usage", "Annotsv Datapath")), ui.column(2,"Launch tool from: "), ui.column(4, ui.output_text_verbatim("text_annotsvusage_dtpt"))),
               ui.hr(),

        ),
        #STARTING UI PAGE
        ui.nav("Start", ui.h1("Pipeline Setup"), ui.hr(),
...
def server(input, output, session):
    ...
    with open(config_path) as fp:
        data = yaml.load(fp)
    dorado_path = reactive.Value(data["tool_usage"]["dorado"])
    dorado_model_path = reactive.Value(data["tool_usage"]["dorado_mdl"])
    clair3_path = reactive.Value(data["tool_usage"]["clair3"])
    gasoline_path = reactive.Value(data["tool_usage"]["gasoline"])
    annotsv_path = reactive.Value(data["tool_usage"]["annotsv"])
    ...
    @reactive.Effect
    @reactive.event(input.dorado_usage)
    def _():
        cnct_bttn.set("connect_dorado_usage")

    @reactive.Effect
    @reactive.event(input.dorado_mdl_folder)
    def _():
        cnct_bttn.set("connect_dorado_mdl_folder")

    @reactive.Effect
    @reactive.event(input.clair3_usage)
    def _():
        cnct_bttn.set("connect_clair3_usage")

    @reactive.Effect
    @reactive.event(input.gasoline_usage)
    def _():
        cnct_bttn.set("connect_gasoline_usage")

    @reactive.Effect
    @reactive.event(input.annotsv_usage)
    def _():
        cnct_bttn.set("connect_annotsv_usage")
    ...
    @reactive.Effect
    @reactive.event(input.connect_dorado_usage)
    def ref19_dtpt():
        server_utils.tool_dtpt(input.ssh(), input.user(), input.password(), tmp_IP, tmp_user, dorado_path, "dorado", data, config_path)

    @reactive.Effect
    @reactive.event(input.connect_dorado_mdl_folder)
    def ref19_dtpt():
        server_utils.tool_dtpt(input.ssh(), input.user(), input.password(), tmp_IP, tmp_user, dorado_model_path, "dorado_mdl", data, config_path)

    @reactive.Effect
    @reactive.event(input.connect_clair3_usage)
    def ref19_dtpt():
        server_utils.tool_dtpt(input.ssh(), input.user(), input.password(), tmp_IP, tmp_user, clair3_path, "clair3", data, config_path)

    @reactive.Effect
    @reactive.event(input.connect_gasoline_usage)
    def ref19_dtpt():
        server_utils.tool_dtpt(input.ssh(), input.user(), input.password(), tmp_IP, tmp_user, gasoline_path, "gasoline", data, config_path)

    @reactive.Effect
    @reactive.event(input.connect_annotsv_usage)
    def ref19_dtpt():
        server_utils.tool_dtpt(input.ssh(), input.user(), input.password(), tmp_IP, tmp_user, annotsv_path, "annotsv", data, config_path)

    @reactive.Effect
    @reactive.event(input.connect_ref_path_19)
    def ref19_dtpt():
        server_utils.ref_dtpt(input.ssh(), input.user(), input.password(), tmp_IP, tmp_user, hg19_path, "ref_hg19_path", data, config_path)

    @reactive.Effect
    @reactive.event(input.connect_ref_path_38)
    def ref38_dtpt():
        server_utils.ref_dtpt(input.ssh(), input.user(), input.password(), tmp_IP, tmp_user, hg38_path, "ref_hg38_path", data, config_path)
    ...
    @render.text
    def text_doradousage_dtpt():
        return server_utils.text_dtpt(dorado_path, "Undefined Usage")
    
    @render.text
    def text_doradomdlfold_dtpt():
        return server_utils.text_dtpt(dorado_model_path, "Undefined Model Folder")

    @render.text
    def text_clair3usage_dtpt():
        return server_utils.text_dtpt(clair3_path, "Undefined Usage")

    @render.text
    def text_gasolineusage_dtpt():
        return server_utils.text_dtpt(gasoline_path, "Undefined Usage")
    
    @render.text
    def text_annotsvusage_dtpt():
        return server_utils.text_dtpt(annotsv_path, "Undefined Usage")
    ...
    def gonangen():
        launch_gonangen(data, workflow_dir, workdir, in_path, input.threads_num(), input.sampsize(), input.ref(), hg19_path, hg38_path, svnano, snvnano,
